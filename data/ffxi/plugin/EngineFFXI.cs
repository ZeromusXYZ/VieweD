        ...
        private bool LoadFromSqLite3(PacketList packetList, string sqliteFileName)
        {
            ...
            using (LoadingForm loadForm = new LoadingForm(MainForm.ThisMainForm))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show("Exception: " + ex.Message, "Exception loading SQLite file", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
            }
        ...
        public override bool LoadFromStream(PacketList packetList, Stream fileStream, string sourceFileName, string rulesFileName, string decryptVersion)
        {
            ...
            if ((expectedLogType == "WindowerPacketViewer") || (expectedLogType == "AshitaPacketeer") || (expectedLogType == "Clipboard"))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        //ErrorMessages.Add("This program does not support file paths that are longer than MAX_PATH (260 characters by default)");
                        //ErrorMessages.Add("Please consider shortening your directory or file names, and try again.");
                    ...
                    {
                        // ErrorMessages.Add(ex.Message);
                        MessageBox.Show("Exception:\r\n" + ex.Message, "LoadFromFile()", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    }
                    return false;
                }
            }
