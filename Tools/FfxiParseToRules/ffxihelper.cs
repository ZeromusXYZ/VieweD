...
namespace FfxiParseToRules
{
    internal class ffxihelper
    {
        private static string ToDoComment(string text)
        {
        ...
        private static string AddTag(string tagName, string text)
        {
            if (string.IsNullOrWhiteSpace(text))
                return string.Empty;
        ...
        private static string AddTag(string tagName, int number)
        {
            if (number == 0)
                return string.Empty;

            return tagName + "=\"" + number + "\" ";
        }

        public static string FileDescription { get; set; } = string.Empty;
        public static string FilePacketName { get; set; } = string.Empty;
        ...
        public static string ConvertLine(string line)
        {
            ...
            {
                lineComment = line.Substring(commentStartPos + 2);
            ...
            if (line.StartsWith("[[") && line.EndsWith("]]"))
            {
                ...
                return ToDoComment(thisSwitchName);
            }
            ...
            if (typeField == "file")
            {
                FilePacketName = nameField;
                FileDescription = descriptionField;
                return string.Empty;
            }
            ...
            if (typeField == "save")
            {
                ...
                if (fields.Count() < 4)
                {
                    // Not enough fields for save function, requires 3 parameters
                    return ToDoComment(line);
                }
                ...
                return "<lookup save=\"" + fields[1] + "\" source=\"" + fields[2] + "\" val=\"" + fields[3] + "\" " + AddTag("comment", lineComment) + "/>";
            }
            ...
            if (posSplitPos > 0)
            {
                ...
                if (rangeSplitPos > 0)
                {
                    ...
                    var subOffsetRangeStr =
                        subOffsetStr.Substring(rangeSplitPos + 1, subOffsetStr.Length - rangeSplitPos - 1);
            ...
            if (typeField == "switchblock")
            {
                ...
                if (AllowAutoSwitchBlock)
                {
                    ...
                    if (ValueInStringList(posVal, descriptionField))
                    {
                        ...
                        // Debug Info
                        if (enableCode)
                            AddParseLineToView(0xFF, "L " + parseLineNumber.ToString(), Color.Red, "SwitchBlock",
                                "Activate Block: " + ActiveSwitchBlock);
            ...
            else if (typeField == "showblock")
            {
                ...
                if (AllowAutoSwitchBlock)
                {
                    ...
                    // Debug Info
                    if (enableCode)
                        AddParseLineToView(0xFF, "L " + parseLineNumber.ToString(), Color.Red, "ShowBlock",
                            "Activate Block: " + ActiveSwitchBlock);
            ...
            else if (typeField == "info")
            {
                ...
                else
                {
                    ...
                    return "<!-- " + Offset + " " + nameField + " " + descriptionField + " -->";
            ...
            else if ((typeField == "byte") || (typeField == "uint8"))
            {
                return "<data type=\"byte\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                var d = PD.GetByteAtPos(Offset);
                var l = Lookup(lookupField, (UInt64)(d), lookupOffsetEvalString);
                AddDataField(Offset, 1);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    l + d.ToString() + " - 0x" + d.ToString("X2") + " - " + ByteToBits(d) + " - '" + (char)d + "'",
                    descriptionField);
                MarkParsed(Offset, 1, DataFieldIndex);
                */
            }
            else if ((typeField == "sbyte") || (typeField == "int8"))
            {
                return "<data type=\"sbyte\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                var d = PD.GetSByteAtPos(Offset);
                var l = Lookup(lookupField, (UInt64)unchecked((byte)d), lookupOffsetEvalString);
                AddDataField(Offset, 1);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    l + d.ToString() + " - 0x" + d.ToString("X2") + " - " + ByteToBits(unchecked((byte)d)) + " - '" +
                    (char)d + "'", descriptionField, (UInt64)unchecked((byte)d));
                MarkParsed(Offset, 1, DataFieldIndex);
                */
            }
            else if ((typeField == "bit") || (typeField == "bool"))
            {
                return "<data type=\"bits\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("bit", SubOffset) + " bits=\"1\" style=\"compact\" " + AddTag("comment", descriptionField) + "/>";
                /*
                ...
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField, d.ToString(),
                    descriptionField, (UInt64)(d ? 1 : 0));
                MarkParsed(Offset, 1, DataFieldIndex);
                */
            }
            else if (typeField == "bits")
            {
                return "<data type=\"bits\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("bit", SubOffset) + " bits=\""+ SubOffsetRange + "\" style=\"compact\" " + AddTag("lookup", lookupField) + AddTag("comment", descriptionField) + "/>";
                /*
                ...
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    l + "0x" + d.ToString("X") + " - " + d.ToString(""), descriptionField, (UInt64)d);
                MarkParsed(firstbyte, bytesize, DataFieldIndex);
                */
            }
            else if (typeField == "uint16")
            {
                return "<data type=\"uint16\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                var d = PD.GetUInt16AtPos(Offset);
                var l = Lookup(lookupField, (UInt64)(d), lookupOffsetEvalString);
                AddDataField(Offset, 2);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    l + d.ToString() + " (0x" + d.ToString("X4") + ")", descriptionField);
                MarkParsed(Offset, 2, DataFieldIndex);
                */
            }
            else if (typeField == "int16")
            {
                return "<data type=\"int16\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                var d = PD.GetInt16AtPos(Offset);
                var l = Lookup(lookupField, (UInt64)unchecked((UInt16)d), lookupOffsetEvalString);
                AddDataField(Offset, 2);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    l + d.ToString() + " (0x" + d.ToString("X4") + ")", descriptionField, (UInt64)unchecked((UInt16)d));
                MarkParsed(Offset, 2, DataFieldIndex);
                */
            }
            else if (typeField == "uint32")
            {
                return "<data type=\"uint32\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                var d = PD.GetUInt32AtPos(Offset);
                var l = Lookup(lookupField, (UInt64)(d), lookupOffsetEvalString);
                AddDataField(Offset, 4);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    l + d.ToString() + " (0x" + d.ToString("X8") + ")", descriptionField, d);
                MarkParsed(Offset, 4, DataFieldIndex);
                */
            }
            else if (typeField == "int32")
            {
                return "<data type=\"int32\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                var d = PD.GetInt32AtPos(Offset);
                var l = Lookup(lookupField, (UInt64)unchecked((UInt32)d), lookupOffsetEvalString);
                AddDataField(Offset, 4);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    l + d.ToString() + " (0x" + d.ToString("X8") + ")", descriptionField, (UInt64)unchecked((UInt32)d));
                MarkParsed(Offset, 4, DataFieldIndex);
                */
            }
            else if (typeField == "float")
            {
                return "<data type=\"float\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
            ...
            else if (typeField == "pos")
            {
                return "<data type=\"pos\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                ...
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    "X:" + x.ToString("F") + "  Y:" + y.ToString("F") + "  Z:" + z.ToString("F"), descriptionField);
                MarkParsed(Offset, 12, DataFieldIndex);
                */
            }
            else if (typeField == "dir")
            {
                return "<data type=\"dir\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                var d = PD.GetByteAtPos(Offset);
            ...
            else if (typeField.StartsWith("string"))
            {
                ...
                return "<data type=\"t\" name=\"" + nameField + "\" " + AddTag("arg", size) + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                ...
                else if (Engines.ShowStringHexData)
                {
                    foreach (char c in d)
                    {
            ...
            else if (typeField == "vstring")
            {
                return "<data type=\"zu8\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                ...
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField, stringdata,
                    descriptionField);
                MarkParsed(Offset, stringSize + 2, DataFieldIndex);
                */
            }
            else if (typeField == "ffstring")
            {
                return ToDoComment(line);
                /*
            ...
            else if (typeField.StartsWith("data"))
            {
                ...
                return "<data type=\"a\" name=\"" + nameField + "\" arg=\"" + size + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
            ...
            else if (typeField == "ms")
            {
                return "<data type=\"ms\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                var d = PD.GetUInt32AtPos(Offset);
                AddDataField(Offset, 4);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    MSToString(d) + " (" + d.ToString() + ")", descriptionField);
                MarkParsed(Offset, 4, DataFieldIndex);
                */
            }
            else if (typeField == "frames")
            {
                return "<data type=\"uint32\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", "frames") + "/>";
                /*
                var d = PD.GetUInt32AtPos(Offset);
                AddDataField(Offset, 4);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    FramesToString(d) + " (" + d.ToString() + ")", descriptionField);
                MarkParsed(Offset, 4, DataFieldIndex);
                */
            }
            else if (typeField == "vanatime")
            {
                return ToDoComment(line);
                /*
                var d = PD.GetUInt32AtPos(Offset);
                ...
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    vt.LocalEarthTime.ToString("yyyy-MM-dd HH:mm:ss") + " (" + d.ToString() + ") => " + vt.ToString(),
                    descriptionField);
                MarkParsed(Offset, 4, DataFieldIndex);
                */
            }
            else if (typeField == "ip4")
            {
                return "<data type=\"ip4\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("comment", descriptionField) + "/>";
                /*
                var d = PD.GetIP4AtPos(Offset);
                AddDataField(Offset, 4);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    d + " <= " + PD.GetUInt32AtPos(Offset), descriptionField);
                MarkParsed(Offset, 4, DataFieldIndex);
                */
            }
            else if (typeField == "linkshellstring")
            {
                return ToDoComment(line);
                /*
            ...
            else if (typeField == "inscribestring")
            {
                return ToDoComment(line);
                /*
            ...
            else if (typeField == "bitflaglist")
            {
                // Default to 64 bits if nothing provided
                if (SubOffsetRange <= 0)
                    SubOffsetRange = 64;

                return "<data type=\"bits\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("bit", SubOffset) + "bits=\"" + SubOffsetRange + "\" style=\"normal\" " + AddTag("comment", descriptionField) + "/>";
                ...
                while (posByte + (posBit / 8) < PD.RawBytes.Count)
                {
                    ...
                    if (thisBit)
                    {
                        ...
                        AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex),
                            nameField + " - Bit" + bitNumber.ToString(), d, descriptionField);
                    }
                ...
                {
                    AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                        "No bits are set", descriptionField);
                }

                MarkParsed(Offset, SubOffsetRange / 8, DataFieldIndex);
                */
            }
            else if (typeField == "bitflaglist2")
            {
                // Default to 16 bits if nothing provided
                if (SubOffsetRange <= 0)
                    SubOffsetRange = 16;

                return "<data type=\"bits\" name=\"" + nameField + "\" " + AddTag("pos", Offset) + AddTag("bit", SubOffset) + "bits=\"" + SubOffsetRange + "\" style=\"compact\" " + AddTag("comment", descriptionField) + "/>";
            ...
            else if (typeField == "combatskill")
            {
                ...
                if ((d & 0x8000) != 0)
                    cappedString = " (Capped)";
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    skilllevel.ToString() + cappedString + " <= 0x" + d.ToString("X4"), descriptionField);
                MarkParsed(Offset, 2, DataFieldIndex);
                */
            }
            else if (typeField == "craftskill")
            {
                return ToDoComment(line);
                /*
                var d = PD.GetUInt16AtPos(Offset);
                ...
                if ((d & 0x8000) != 0)
                    cappedString = " (Capped)";
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    "Level: " + craftlevel.ToString() + " Rank: " + craftrank.ToString() + " " +
                    ffxiLookup.NLU(EngineFFXI.LU_CraftRanks).GetValue((UInt64)craftrank) + cappedString + " <= 0x" +
                    d.ToString("X4"), descriptionField);
                MarkParsed(Offset, 2, DataFieldIndex);
                */
            }
            else if (typeField == "equipsetitem")
            {
                return ToDoComment(line);
                /*
                AddDataField(Offset, 4);
                ...
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    "0x" + PD.GetUInt32AtPos(Offset).ToString("X4") + " => " + d, descriptionField);
                MarkParsed(Offset, 4, DataFieldIndex);
                */
            }
            else if (typeField == "equipsetitemlist")
            {
                return ToDoComment(line);
                /*
                ...
                for (int off = 0; (off < PD.RawBytes.Count - 4) && (c < counter); off += 4)
                {
                    ...
                    d += "Item: " + ffxiLookup.NLU(EngineFFXI.LU_Item).GetValue((UInt64)item);
                    AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex),
                        nameField + " #" + c.ToString(),
                        "0x" + PD.GetUInt32AtPos(off + Offset).ToString("X4") + " => " + d, descriptionField);
                    c++;
                }
            ...
            else if (typeField == "abilityrecastlist")
            {
                return ToDoComment(line);
                /*
                string d = "";
                ...
                for (int c = 0; c < counter; c++)
                {
                    ...
                    {
                        AddDataField(off, 8);
                        AddParseLineToView(DataFieldIndex, "0x" + off.ToString("X2"), GetDataColor(DataFieldIndex),
                            nameField + " #" + c.ToString(), d, descriptionField);
            ...
            else if (typeField == "blacklistentry")
            {
                return ToDoComment(line);
                /*
                ...
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField,
                    "ID: 0x" + pID.ToString("X8") + " => " + pName, descriptionField);
                MarkParsed(Offset, 20, DataFieldIndex);
                */
            }
            else if (typeField == "meritentries")
            {
                return ToDoComment(line);
                /*
                // Special field, SubOffset is used a position for the count byte
                ...
                for (int off = 0; (off < PD.RawBytes.Count - 4) && (c < counter); off += 4)
                {
                    ...
                    string d = "";
                    d += "ID: 0x" + meritID.ToString("X4") + " (" +
                         ffxiLookup.NLU(EngineFFXI.LU_Merit).GetValue((UInt64)meritID) + ") - ";
            ...
            else if (typeField == "playercheckitems")
            {
                return ToDoComment(line);
                /*
                ...
                while ((n <= (PD.RawBytes.Count - 28)) && (c < counter))
                {
                    ...
                    string idStr = "0x" + idVal.ToString("X4") + " => " +
                                   ffxiLookup.NLU(EngineFFXI.LU_Item).GetValue(idVal);
                    string slotStr = "0x" + slotVal.ToString("X4") + " => " +
                                     ffxiLookup.NLU(EngineFFXI.LU_EquipmentSlots).GetValue(slotVal);
                    string unkStr = "0x" + unkVal.ToString("X4") + " (" + unkVal.ToString() + ")";
                    string extdataStr = PD.GetDataAtPos(n + 4, 24);

                    AddDataField(n, 28);
                    AddParseLineToView(DataFieldIndex, "0x" + n.ToString("X"), GetDataColor(DataFieldIndex),
                        nameField + " #" + c.ToString() + " ID", idStr, descriptionField);
                    AddParseLineToView(DataFieldIndex, "0x" + n.ToString("X"), GetDataColor(DataFieldIndex),
                        nameField + " #" + c.ToString() + " Slot", slotStr, descriptionField);
                    AddParseLineToView(DataFieldIndex, "0x" + n.ToString("X"), GetDataColor(DataFieldIndex),
                        nameField + " #" + c.ToString() + " ???", unkStr, descriptionField);
                    AddParseLineToView(DataFieldIndex, "0x" + n.ToString("X"), GetDataColor(DataFieldIndex),
                        nameField + " #" + c.ToString() + " ExtData", extdataStr, descriptionField);
                    MarkParsed(n, 28, DataFieldIndex);
                    n += 28;
                }
                */
            }
            else if (typeField == "bufficons")
            {
                return ToDoComment(line);
                /*
                ...
                while ((n <= PD.RawBytes.Count - 2) && (c < counter))
                {
                    ...
                    string iconStr = "0x" + iconVal.ToString("X4") + " => " +
                                     ffxiLookup.NLU(EngineFFXI.LU_Buffs).GetValue(iconVal);

                    AddDataField(n, 2);
                    AddParseLineToView(DataFieldIndex, "0x" + n.ToString("X"), GetDataColor(DataFieldIndex),
                        nameField + " #" + c.ToString(), iconStr, descriptionField);
                    MarkParsed(n, 2, DataFieldIndex);
                    n += 2;
                }
                */
            }
            else if (typeField == "bufftimers")
            {
                return ToDoComment(line);
                /*
                // SubOffset is counter to use, defaults to 1
                ...
                while ((n <= PD.RawBytes.Count - 4) && (c < counter))
                {
                    ...
                    AddDataField(n, 4);
                    AddParseLineToView(DataFieldIndex, "0x" + n.ToString("X"), GetDataColor(DataFieldIndex),
                        nameField + " #" + c.ToString(), timerStr, descriptionField);
                    MarkParsed(n, 4, DataFieldIndex);
                    n += 4;
                }
                */
            }
            else if (typeField == "buffs")
            {
                return ToDoComment(line);
                /*
                // SubOffset is counter to use, defaults to 1
                ...
                while ((n <= PD.RawBytes.Count - 2) && (t <= PD.RawBytes.Count - 4) && (c < counter))
                {
                    ...
                    AddDataField(n, 2);
                    AddParseLineToView(DataFieldIndex, "0x" + n.ToString("X"), GetDataColor(DataFieldIndex),
                        nameField + " #" + c.ToString() + " Icon", iconStr, descriptionField);
                    MarkParsed(n, 2, DataFieldIndex);
                    ...
                    // AddDataField(t, 4);
                    AddParseLineToView(DataFieldIndex, "0x" + t.ToString("X"), GetDataColor(DataFieldIndex),
                        nameField + " #" + c.ToString() + " Timer", timerStr, descriptionField);
            ...
            else if (typeField == "jobpointentries")
            {
                return ToDoComment(line);
                /*
                ...
                while ((n <= PD.RawBytes.Count - 4) && (c < counter))
                {
                    ...
                    d += "ID: 0x" + jpVal.ToString("X4") + " => " +
                         ffxiLookup.NLU(EngineFFXI.LU_JobPoint).GetValue(jpVal) + " + ";
            ...
            else if (typeField == "guildshopitems")
            {
                return ToDoComment(line);
                /*
                int counter = 30;
            ...
            else if (typeField == "roequest")
            {
                ...
                if (maxVal == "")
                    maxVal = "???";
                string d = "";
                d += "ID; 0x" + idVal.ToString("X3") + " => " +
                     PD.Parent.ParentTab.Engine.DataLookups.NLU(EngineFFXI.LU_RoE).GetValue((UInt64)idVal) + "  ";
            ...
            else if (typeField == "dialogtext")
            {
                return ToDoComment(line);
                /*
                var d = PD.GetUInt16AtPos(Offset);
                var l = "0x" + d.ToString("X4") + " => " +
                        EngineFFXI.DialogsListFfxi.GetValue((ushort)PD.CapturedZoneId, d);
                AddDataField(Offset, 2);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField, l + d.ToString(),
                    descriptionField);
                MarkParsed(Offset, 2, DataFieldIndex);
                */
            }
            else if (typeField == "fishrankservermessage")
            {
                return ToDoComment(line);
                /*
                ...
                AddDataField(Offset, 4);
                AddParseLineToView(DataFieldIndex, posField, GetDataColor(DataFieldIndex), nameField + " count", l,
                    descriptionField);
                MarkParsed(Offset, 4, DataFieldIndex);
                ...
                for (int i = 0; i < fishrankentrycountforloop; i++)
                {
                    ...
                    else
                    {
                        ...
                        fishline += " - ";
                        var job = ffxiLookup.NLU(EngineFFXI.LU_Job).GetValue(PD.GetByteAtPos(thisoffset + 16)) +
                                  PD.GetByteAtPos(thisoffset + 18).ToString("00");
                        ...
                        fishline += " ";
                        fishline += ffxiLookup.NLU("raceandgender").GetValue(PD.GetByteAtPos(thisoffset + 20), "?")
                            .PadRight(10); // race
                        fishline += " ";
                        fishline += ffxiLookup.NLU("nations").GetValue(PD.GetByteAtPos(thisoffset + 22), "Unknown")
                            .PadRight(10); // Allegience
                    ...
                    var fishFieldName = nameField + " " + (i + 1).ToString() + "/" +
                                        fishrankentrycountforloop.ToString();
                    if (i == 0)
                        fishFieldName = nameField + " self";
                    AddParseLineToView(DataFieldIndex, "0x" + thisoffset.ToString("X2"), GetDataColor(DataFieldIndex),
                        fishFieldName.ToString(), fishline, descriptionField);
                    MarkParsed(thisoffset, 36, DataFieldIndex);
                }
                */
            }
            else if (typeField == "packet-in-0x028")
            {
                return ToDoComment(line);
                /*
            ...
            else
            {
                ...
                AddParseLineToView(0xFF, "L " + parseLineNumber.ToString(), GetDataColor(DataFieldIndex), "Parse Error",
                    "Unknown Field Type: " + typeField);
                */
            }
        }
    }
